...
function HelpModal(props){
  var sections=[
    {title:"📋 Board View — Your Daily Workspace",body:"The default view when you log in. Tasks are in three columns: To Do, In Progress, and Done.\n\n• Click any task card to expand it and see notes, subtasks, and actions\n• Move a task between columns using the arrow buttons inside the expanded card\n• Mark a task done by clicking > Done inside the card\n• Recurring tasks appear automatically on the day they are due — no action needed\n• Use the filter bar below the top bar to sort by Priority or Due Date, filter to Past Due only, or narrow to Today / This Week / This Month / This Quarter\n• The 'Clear all' button on the Done column permanently removes completed tasks"},
    {title:"📅 Monthly View — Calendar Overview",body:"A full month calendar showing all your tasks placed on their relevant date.\n\n• Tasks with a due date appear on that date\n• Tasks without a due date appear on the day they were created\n• Recurring tasks are shown up to 18 months ahead on their repeat date (not their deadline)\n• Color of each task pill matches its category (green = Nuve, yellow = Kesos, blue/purple = Personal)\n• Click any task pill to open and edit it\n• Use the arrows at the top to navigate between months\n• The sidebar category and 'Assigned to' filters apply here too"},
    {title:"📊 Quarterly View — Big Picture Planning",body:"Shows all four quarters of the selected year in a side-by-side grid, so you can see what's coming across the whole year.\n\n• Each task appears in the quarter its date falls in\n• Recurring tasks are automatically expanded — every occurrence shows in its correct quarter\n• The colored dot on each task matches its category color\n• Use the year arrows at the top to navigate forward or back\n• The sidebar category and 'Assigned to' filters apply here too\n• Tip: this view is best for spotting busy periods and planning ahead"},
    {title:"🔁 Recurring View — Manage Repeating Tasks",body:"Lists every recurring task as a single row — one entry per task, no duplicates.\n\n• Shows: repeat schedule, active date, deadline (if set), next cycle date, and when it was last completed\n• Edit a recurring task here to change its title, schedule, category, assignee, or deadline — changes take effect for all future cycles\n• Delete a recurring task from here to remove it permanently for all users (requires confirmation)\n• This is the only place to delete a recurring task — individual board completions do not delete the series"},
    {title:"📝 Notes — Scratchpad & Shared Docs",body:"A standalone notes section, separate from tasks. Access it via the Notes button in the top bar.\n\nThree tabs:\n• Private — only you can see these. Good for personal reminders or drafts\n• Context — notes tied to a specific category (e.g. a running doc for Nuve Operations)\n• Shared — a live two-way scratchpad between you and one other user. Both people can read and write\n\nAll notes auto-save as you type. You can create as many notes as you need in each tab."},
    {title:"✅ Creating & Editing Tasks",body:"Tap '+ New task' in the top bar to create a task.\n\nFor a one-time task:\n• Fill in title, category, priority, due date, and who it's assigned to\n• Optionally add subtasks, shared notes, private notes (only you see them), or a message for the assignee (they see it when they open the task)\n\nFor a recurring task:\n• Choose a repeat schedule (e.g. Every Wednesday)\n• Set a 'Repeat starts' date — the first day the task becomes active\n• Optionally set a deadline offset (e.g. 2 days after repeat = Friday deadline)\n• The due date field is replaced by these two fields for recurring tasks\n\nTo edit an existing task, click the pencil icon on the card. All changes save immediately."},
    {title:"🔄 How Recurring Tasks Work",body:"Recurring tasks are designed to be zero-maintenance:\n\n• They appear on the Board automatically on their repeat date\n• If a deadline offset is set, the 'for [date]' badge on the card shows the active start date\n• Mark it done using > Done inside the card — it disappears from the board and comes back automatically on the next cycle\n• If a recurring task is not completed by its deadline, it stays on the board with a red PAST DUE badge\n• To permanently stop a recurring task, go to the Recurring tab and click Delete\n• On the Monthly and Quarterly views, all future occurrences are shown on their repeat date so you can see what's coming"},
    {title:"🗂 Sidebar — Categories & Filters",body:"The sidebar on the left controls what you see in every view.\n\nCategory filter:\n• Click 'All tasks' to see everything you have access to\n• Click a parent category (Nuve, Kesos Tacos, Personal) to see all tasks in that group\n• Click a subcategory (e.g. Operations, Finances) to narrow to just that section\n\nAssigned to filter:\n• Select 'Everyone' to see all tasks regardless of who they're assigned to\n• Select a specific person to see only their tasks\n\nThese filters apply to Board, Monthly, Quarterly, and Recurring views. On mobile, tap the ☰ icon to open and close the sidebar."},
    {title:"🏷 Task Badges & What They Mean",body:"Each task card shows colored badges that give you quick context:\n\n• Category badge — colored by category (green = Nuve, yellow = Kesos, etc.)\n• Due date — calendar icon with the date; turns red if overdue\n• 'for [date]' blue badge — on recurring tasks, shows the active repeat date for this cycle\n• PAST DUE — red badge, task has passed its deadline\n• Every [day] — green recurring badge showing the repeat schedule\n• Shared — purple badge on tasks shared between Jhonatan and Sarah\n• X/Y done — shows subtask progress\n• Avatar icons — show who the task is assigned to and who created it"},
    {title:"👥 Users & What Each Person Sees",body:"Each user logs in with their name and 4-digit PIN.\n\nJhonatan — full access to everything: Nuve, Kesos Tacos, and all Personal categories\n\nSarah — sees Personal & Rentals categories, plus her own 'My Work' section (SW: categories). Also sees tasks shared between her and Jhonatan.\n\nGin — sees Nuve and Kesos Tacos categories only. Has a special 'View as Jhonatan' button in the top bar — when activated, she can see, create, and complete Jhonatan's Nuve and Kesos tasks on his behalf (Personal tasks are always hidden from Gin).\n\nAll users: tap the sun/moon icon to toggle dark mode. Tap the download icon to export your tasks as a CSV spreadsheet."},
    {title:"🔍 Search & Shortcuts",body:"Search:\n• Click the 🔍 icon in the top bar, or press / on a keyboard, to open the search bar\n• Search filters by task title, notes, and category across the current view\n• Press Escape to close search\n\nKeyboard shortcuts (desktop):\n• N — open New task\n• / — toggle search\n• Escape — close any open panel or modal\n\nNotification bell:\n• A red bell icon appears when someone has left a message for you on a task\n• Click it to go straight to your tasks\n• The message appears as a blue '📌 Note for you' when you expand the task"},
  ];
    ...
    ce("div",{onClick:function(e){e.stopPropagation();},style:{background:WH,borderRadius:16,width:"100%",maxWidth:580,boxShadow:"0 16px 48px rgba(0,0,0,.18)"}},
      ...
      ce("div",{style:{padding:"12px 16px 20px"}},
        ...
        ce("div",{style:{marginTop:16,padding:"12px 16px",background:"#E8FBF1",borderRadius:10,fontSize:12,color:MD,lineHeight:1.6,textAlign:"center"}},
          "💡 The sidebar filter applies to all views. When in doubt, click 'All tasks' and 'Everyone' to reset and see everything."
        )
      )
    )
  );
